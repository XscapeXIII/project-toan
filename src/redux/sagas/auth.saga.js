...
function* registerSaga(action) {
  try {
    const { data, callback } = action.payload;
    const result = yield axios.post("http://localhost:5000/register", data);
...
function* updateUserInfoSaga(action) {
  try {
    ...
    const result = yield axios.patch(`http://localhost:5000/users/${id}`, data);
    yield put({
      type: SUCCESS(AUTH_ACTION.GET_USER_INFO),
      payload: {
        data: result.data,
...
function* changePasswordSaga(action) {
  try {
    ...
    const result = yield axios.patch(`http://localhost:5000/users/${id}`, {
